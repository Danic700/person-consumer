...
public class PersonConsumerService{
    ...
    @RabbitListener(queues = {"person.queue", "deadLetter.queue"})
    public void consumeMessage(Person person){
        logger.info("Received Message From RabbitMQ Person Exchange: " + person);
        try {
            HttpUtil.sendHttpPostRequest(dbServiceUrl, person);
            logger.info("Message successfully sent to db service: " + person);
        }
        catch (RestClientException e){
            logger.error("Couldnt send Message to db-service for "+person+", will now throw exception and retry according to logic in config");
            throw e;
        }
    }
